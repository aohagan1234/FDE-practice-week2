...
| `in_active_outreach` state is consistent and readable in real time across all concurrent fill cycles | Medium | Rules 1 and 3 fail; candidates may be double-contacted across concurrent fills, wasting the outreach window. **Validate with Aaron (week 1).** Fallback if no shared state infrastructure exists: implement as a database flag per nurse — achieves the same exclusion without architectural change |
| Urgency tier thresholds (≤4h, 4–24h, >24h) and corresponding outreach windows (20 / 60 / 120 min) reflect actual MedFlex nurse response patterns | Low | Outreach windows expire too early or too late for a material proportion of fills; fill time target is missed or pool is burned through prematurely. **Thresholds and windows are configurable defaults** — validate against pilot data from weeks 2–5 and adjust without architectural change. Validate with Kim. |
