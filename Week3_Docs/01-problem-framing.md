...
|---|---|---|---|
| Fill time 4.2h → <1h | Parallel outreach replaces sequential outreach. The agent contacts multiple candidates simultaneously; the coordinator is removed from the outreach loop and touches the fill cycle only at intake review and final approval (~5 min active time vs. ~20 min today). The waiting time between sequential contact attempts — which is where the 4.2 hours comes from — is eliminated. | D3 before/after lifecycle diagrams; D3 parallel outreach step (Agent-led); ADR 1 (coordinator gate at final step only); ADR 2 (agentic ranking with stated rationale builds coordinator trust in recommendations) | Assumes fill time is predominantly waiting time, not decision complexity — Low confidence; to validate with Kim |
