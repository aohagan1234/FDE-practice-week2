...
| Linda (compliance, not in session) | Credential accuracy; state regulatory requirements; quarterly update cadence | Defines credential data structure and update frequency; compliance gap is the largest mismatch risk |
...
| Credential data lag (quarterly update cadence creates placement window for lapsed credentials) | H | H | Agent checks credential status at placement time; excludes candidates with credentials last verified >90 days; flags to compliance team |
